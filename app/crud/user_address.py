...
def _to_out(doc: dict) -> UserAddressOut:
    return UserAddressOut.model_validate(doc)


async def create(payload: UserAddressCreate) -> UserAddressOut:
    # Keep ObjectIds as ObjectIds in Mongo
...
async def get_one(_id: PyObjectId) -> Optional[UserAddressOut]:
    try:
...
async def update_one(_id: PyObjectId, payload: UserAddressUpdate) -> Optional[UserAddressOut]:
    try:
        oid = ObjectId(str(_id))
    except Exception:
        return None
