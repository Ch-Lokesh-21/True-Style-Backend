...
async def create_admin_service(payload:RegisterIn) -> Optional[UserOut]:
    try:
        ...
        if not role:
            raise HTTPException(
                status.HTTP_500_INTERNAL_SERVER_ERROR, "Default user role not found")
        role_id = role["_id"]
        status_doc = await db["user_status"].find_one({"status": "active"})
        if not status_doc:
            raise HTTPException(
                status.HTTP_500_INTERNAL_SERVER_ERROR, "Default user status not found")
...
async def get_users_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list users: {e}")
...
async def update_user_service(
    user_id: PyObjectId, user_status_id: PyObjectId=Form(...)
)->Optional[UserOut]:
    try:
